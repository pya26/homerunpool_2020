<?php
	$team_players = $dbh->prepare("SELECT p.PlayerID, p.FirstName, p.LastName, p.MLBID, p.TeamAbbr FROM league_team_players ltp LEFT JOIN players p ON p.PlayerID = ltp.player_id WHERE ltp.league_id = ? AND ltp.season_id = ? AND ltp.status_id = 'A' AND ltp.team_id = {$team_id} ORDER BY ltp.sort ASC");
			...
			while ($row = $team_players->fetch(PDO::FETCH_ASSOC)) {
				...
				*/
				...
				if($injured_players == 1){
					...
					$player_row .= '<td><a href="https://www.mlb.com/player/'.$mlb_player_slug.'" target="_blank">'.$row['FirstName'][0].'. '.$row['LastName'].'</a>&nbsp;&nbsp;&nbsp;('.$row['TeamAbbr'].')</td>';
